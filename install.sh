...
EOF
    ...
    # 포트 개방
    firewall-cmd --permanent --add-port=80/tcp  # API
...
EOF
    ...
    # SOCKS5 개별 서비스는 init_dongle_config.sh에서 생성됨
    log_info "SOCKS5 개별 서비스는 init_dongle_config.sh에서 생성됩니다..."
    ...
    # SOCKS5 개별 서비스는 init_dongle_config.sh에서 활성화됨
...
start_services() {
    ...
    # 동글 설정 초기화
    log_info "동글 설정 초기화..."
    if [ -x "$PROJECT_DIR/init_dongle_config.sh" ]; then
        log_info "init_dongle_config.sh 실행 중..."
        $PROJECT_DIR/init_dongle_config.sh
    else
        log_warning "init_dongle_config.sh가 없습니다. 나중에 수동으로 실행하세요."
    fi
    ...
    # SOCKS5 개별 서비스 시작
    log_info "SOCKS5 개별 서비스 시작..."
    for i in {11..30}; do
        if [ -f "/etc/systemd/system/dongle-socks5-$i.service" ]; then
            systemctl start dongle-socks5-$i 2>/dev/null || true
        fi
    done
    
    log_info "서비스 상태 확인..."
    systemctl status dongle-toggle-api.service --no-pager || true
    $PROJECT_DIR/scripts/socks5/manage_socks5.sh status || true
}
...
verify_installation() {
    ...
    # SOCKS5 개별 서비스 상태
    for i in {11..18}; do
        if systemctl is-active --quiet dongle-socks5-$i 2>/dev/null; then
            echo "dongle-socks5-$i: active"
        fi
    done
...
print_summary() {
    ...
    echo "$PROJECT_DIR/scripts/socks5/manage_socks5.sh status"
    echo ""
    echo "# API 상태 확인"
    echo "curl http://localhost/status"
    echo ""
    echo "# 프록시 정보 확인"
    echo "$PROJECT_DIR/scripts/utils/check_proxy_ips.sh"
    echo ""
    echo "# 동글 제어"
    ...
    echo "3. SOCKS5 프록시는 10011-10030 포트에서 개별 실행됩니다"
